...
class DetailOrderPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Order order = context.watch<OrderBloc>().state.order;
    DateTime dateTime = DateTime.parse(order.date);
    String date = DateFormat("dd-MMM-yyyy HH:mm:ss").format(dateTime);
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          context.read<OrderBloc>().add(DetailOrderEvent(id: order.id));
        },
        child: BlocConsumer<OrderBloc, OrderState>(
          ...
          builder: (context, state) {
            if (state.status == OrderStatus.loadingGetDetail) {
              return const Center(
